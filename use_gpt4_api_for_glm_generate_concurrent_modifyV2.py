...
def process_line(line):
    ...
    try:
        ...
        data['gpt4turbo_modify_response'] = ''
        ...
        for i in range(3):
            ...
            chat_completion = openai.ChatCompletion.create(model="gpt-4-1106-preview", messages = messages)
        ...
        for j in range(3):
            ...
            # 添加用户输入的提示和响应
            chat_completion2 = openai.ChatCompletion.create(model="gpt-4-1106-preview", messages = messages2)
            gpt_response2 = chat_completion2.choices[0].message.content
            try:
                new_response2 = json.loads(gpt_response2)
            except:
                new_response2 = {"response": ''}
            data['gpt4turbo_modify_response'] = new_response2.get('response', '')
            if data['gpt4turbo_modify_response'] != '':
                break
             
        # 检查答案
        for k in range(2):
            response = data.get('response', '')
            gpt_response = data.get('gpt4turbo_modify_response', '')
            ...
            if temp_response == "yes":
                ...
                data['gpt4turbo_modify_response'] = temp_response
                break
...
output_file = os.path.join(out_dir, "target_file_gpt4turboA1 copy_test.jsonl")
